...
TEST_CASE( "FileBuffer benchmarks", "[!benchmark][filebuffer]" )
{
    SECTION( "File mapping performance" )
    {
        const size_t size = 10'000'000;
        std::vector<char> data = BinaryPattern::sequence( size );
        auto tempFile = TempFile::create( data.data(), size );

        BENCHMARK( "Map 10MB file" )
        {
            return FileBuffer( tempFile.path() );
        };
    }

    SECTION( "Multiple mappings of same file" )
    {
        const size_t size = 1'000'000;
        std::vector<char> data = BinaryPattern::repeated( 'Z', size );
        auto tempFile = TempFile::create( data.data(), size );

        BENCHMARK( "Map same 1MB file 100 times" )
        {
            for( int i = 0; i < 100; i++ )
            {
                FileBuffer buffer( tempFile.path() );
